...
-- file would hold the ALTER TABLE's ACCESS EXCLUSIVE lock on DkimRecord for the
-- whole backfill. Measured against production (1.48M rows): a probe query
-- shaped like the backfill's anti-join ran past 300s. The app pool sets
...
-- scripts/backfill-observation-channels.ts, which builds its supporting index
-- CONCURRENTLY, batches, commits as it goes, and is re-runnable. Run it after
-- deploying this migration. Until it does, the new columns stay NULL and a
