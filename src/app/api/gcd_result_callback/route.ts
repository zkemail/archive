...
function verifyRsaPublicKey(
  ...
  try {
    ...
    if (publicKeyHex.length !== keySizeBytes * 2) {
      console.error(
        `Public key length mismatch: expected ${keySizeBytes} hex characters, got ${publicKeyHex.length}`
      );
      return false;
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!taskId) {
      logger.error('gcd_callback_missing_taskId');
      return Response.json({ error: 'Missing taskId' }, { status: 400 });
    }

    if (success) {
      logger.info('gcd_task_completed', { taskId });
      ...
      console.log(
        `${chalk.blue('🔹 Selector :')}  ${chalk.yellow(metadata.selector)}`
      );
      console.log(
        `${chalk.blue('🔹 Domain   :')}  ${chalk.yellow(metadata.domain)}`
      );
      ...
      if (!isHeaderHash1SignatureValid || !isHeaderHash2SignatureValid) {
        logger.error('gcd_public_key_invalid', { taskId });
...
async function storeCalculationResult(data: {
  ...
  try {
    ...
    const existingDsp = await prisma.domainSelectorPair.findFirst({
      where: {
        domain: domain,
        selector: selector,
      },
    ...
    logger.error('gcd_result_store_failed', {
      taskId: data.taskId,
      error: error instanceof Error ? error.message : String(error),
    });
  }
