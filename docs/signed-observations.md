...
because each one costs a signature. Every response carries `X-Total-Records`,
and `X-Records-Truncated: true` when the cap was hit, so a truncated rotation
history never reads as a complete one. No pair is anywhere near the cap.
